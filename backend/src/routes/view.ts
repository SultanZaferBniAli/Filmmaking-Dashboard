...
export function registerViewRoutes(app: FastifyInstance) {
  app.get('/view/workshops', async () => serializeAllWorkshops());
  app.get('/view/workshops/:id', async (req) => {
  ...
  app.get('/view/trainers', async () => serializeAllTrainers());
  app.get('/view/participants', async () => serializeAllParticipants());
  app.get('/view/feedback', async () => serializeAllFeedback());
}
